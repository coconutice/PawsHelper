...
data_in_out = []
data_row = []
incolumns = ['Animal ID', 'Name', 'DateTime', 'MonthYear', 'Found Location',
...
for idx, row in df_join_sorted.iterrows():
  ...
  if tid == prevId and t == "outcome" and prev == "intake":
      ...
      # append if exist otherwise new datarow
      if len(data_row) != 0:
        if prev == "intake":
            # not complete, although outculmns are nan, extend them
            data_row.extend(df_join_sorted.iloc[idx-1][outcolumns].values.tolist())
        else: pass
        data_in_out.append(data_row)
...
df_in_out = pd.DataFrame(data_in_out, columns=['animal_id', 'name', 'intake_datetime', 'month_year', 'found_location',
...
df_in_out.to_csv("../data/Austin_Animal_Center_Intakes_Outcomes.csv", index=False)
